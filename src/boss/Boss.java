...
public class Boss extends Player implements IBoss, IBossOutfit {
    ...
    public Player getPlayerAttack() {
        ...
        if (this.playerTarger != null && this.playerTarger.isPet && this.playerTarger.isClone
                && ((Pet) this.playerTarger).master != null && ((Pet) this.playerTarger).master.equals(this)) {
            this.playerTarger = null;
        }
        ...
        if (this.zone != null) {
            try {
                if (this.currentLevel == 0) {
                    if (this.parentBoss == null) {
                        int maxZone = Math.min(25, this.zone.map.zones.size());
                        int zoneid = Util.nextInt(0, maxZone - 1);

                        int count = 0;

                        if (this.isZone01SpawnDisabled && maxZone > 2) {

                            // Random từ khu 2 trở lên
                            zoneid = Util.nextInt(2, maxZone - 1);
                            ...
                            while (count < maxZone &&
                                ...
                                zoneid = Util.nextInt(0, maxZone - 1);
                                count++;
                            }
                        }
