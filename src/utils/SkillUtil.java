...
public class SkillUtil {
    ...
    public static int getTimeBienHinh(int level) {// thời gian biến hình
        return level * 60000;
    }
    ...
    public static boolean upSkillPet(List<Skill> skills, int index) {
        ...
        int tempId = oldSkill.template.id;
        int level = oldSkill.point + 1;
        ...
        List<Skill> skillList = template.skillss;
        if (skillList == null || level > skillList.size())
            return false;

        Skill newSkill = new Skill(skillList.get(level - 1));

        if (index == 1)
            newSkill.coolDown = 1000;
