...
public class NhapNgocRong {
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (item.template.id > 14 && item.template.id <= 20) {
            ...
            text.append(ConstFont.BOLD_BLUE).append("Con có muốn biến 7 ").append(item.template.name)
                    .append(" thành\n");
            text.append("1 viên ").append(ItemService.gI().getTemplate((short) (item.template.id - 1)).name)
                    .append("\n");
    ...
    public static void nhapNgocRong(Player player, boolean useDBV) {
        ...
        if (item.template.id > 14 && item.template.id <= 20 && !useDBV) {
            ...
            // Service.gI().sendServerMessage(player, "Hành trang đã đầy, cần một ô trống
            // trong hành trang để nhận vật phẩm");
            // return;
            // }
