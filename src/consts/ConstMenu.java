package consts;

/**
 *
 * @author Admin
 */
public class ConstMenu {

    // index bà hạt mít
    public static final int MENU_PHA_LE = 251003;
    public static final int MENU_CHUYEN_HOA_TRANG_BI = 251004;
    public static final int MENU_PHA_LE_HOA_TRANG_BI = 251005;
    public static final int MENU_NANG_CAP_TRANG_BI = 251006;
    public static final int CHUC_NANG_BHM_KHAC = 251007;
    public static final int BUILD_DO_BHM = 251008;
    public static final int SHOP_BHM = 251009;
    public static final int MENU_KICH_HOAT = 251010;
    public static final int MRNU_DI_CHUYEN = 251011;
    public static final int MENU_ENVENT_20_11 = 251012;
    public static final int MENU_ENVENT_HALLOWEEN = 251013;
    public static final int MENU_PHAN_RA_THAN_LINH = 251014;
    public static final int MENU_XAC_NHAN_PHAN_RA_TL = 251015;
    public static final int MENU_NANG_CAP_VAT_PHAM = 1122;
    // index đại thiên sứ

    public static final int MENU_SHOW = 0;
}
