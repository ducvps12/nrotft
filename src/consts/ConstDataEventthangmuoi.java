...
public class ConstDataEventthangmuoi {
    ...
    public static ConstDataEventthangmuoi gI() {
        if (gI == null) {
            gI = new ConstDataEventthangmuoi();
        }
        return gI;
    }
    ...
    public static boolean isActiveEvent() {
        if (!initsukien) {
            ...
            System.out.println("Star Event TOP 20/10: " + startEvent.getTime());
        ...
        if (System.currentTimeMillis() >= startEvent.getTimeInMillis() && System.currentTimeMillis() <= endEvents.getTimeInMillis()) {
            if (isTraoQua && System.currentTimeMillis() + 60000 >= endEvents.getTimeInMillis()) {
                String sql = "SELECT  player.id as plid, player.name as name, account.thiepchucvip FROM account, player WHERE account.id = player.account_id AND account.create_time > '2025-" + MONTH_OPEN + "-" + DATE_OPEN + " " + HOUR_OPEN + ":" + MIN_OPEN + ":00' AND account.thiepchucvip >= 100000 ORDER BY account.thiepchucvip DESC LIMIT 10;";
    ...
    public static void TraoQuaSuKien(Player pl, int rank) {
        ...
        try ( Connection con2 = DBConnecter.getConnectionServer();  PreparedStatement ps = con2.prepareStatement("SELECT detail FROM moc_thiepchucvip WHERE id = ?")) {
