...
public class ConstDataEventTRANGSUCVIP {
    ...
    public static ConstDataEventTRANGSUCVIP gI() {
        if (gI == null) {
            gI = new ConstDataEventTRANGSUCVIP();
        }
        return gI;
    }
    ...
    public static boolean isActiveEvent() {
        ...
        if (System.currentTimeMillis() >= startEvent.getTimeInMillis() && System.currentTimeMillis() <= endEvents.getTimeInMillis()) {
            if (isTraoQua && System.currentTimeMillis() + 60000 >= endEvents.getTimeInMillis()) {
                String sql = "SELECT  player.id as plid, player.name as name, account.capsuvip FROM account, player WHERE account.id = player.account_id AND account.create_time > '2025-" + MONTH_OPEN + "-" + DATE_OPEN + " " + HOUR_OPEN + ":" + MIN_OPEN + ":00' AND account.capsuvip >= 100000 ORDER BY account.capsuvip DESC LIMIT 10;";
    ...
    public static void TraoQuaSuKien(Player pl, int rank) {
        ...
        try ( Connection con2 = DBConnecter.getConnectionServer();  PreparedStatement ps = con2.prepareStatement("SELECT detail FROM moc_capsule_trang_suc WHERE id = ?")) {
