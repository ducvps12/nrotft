...
public class Mob {
    ...
    public Player getPlayerCanAttack() {
        ...
        try {
            ...
            for (Player pl : players) {
                if (!pl.isDie() && !pl.isBoss && !pl.isNewPet && !pl.nPoint.buffDefenseSatellite
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) throws Exception {
        ...
        // Map up vàng
        if (mapid == 180) {
            int vangUpMap = 50_000;
            if (player.nPoint.tlGold > 0) {
                vangUpMap = (int) (vangUpMap * (1 + player.nPoint.tlGold / 100.0));
            }
            list.add(new ItemMap(
                    zone,
                    190,
        ...
        if (player.itemTime.isCoBonLa) {
            if (MapService.gI().isMapUpSKH(mapid)) {
                ...
                if (Util.isTrue(1, 300)) {
                    ...
                    // " vừa nhặt được "
                    // + it.itemTemplate.name + " Sét Kích Hoạt");
                }
            }
        } else {
            if (MapService.gI().isMapUpSKH(mapid)) {
        ...
        if (MapService.gI().isMapPhoBan(mapid) && EventManager.LUNNAR_NEW_YEAR) {
            ...
            // Rơi vật phẩm ID 748
            if (Util.isTrue(10 + levelPlayer, 100)
                    || (player.actived() && Util.isTrue(dropRate + levelPlayer / 2, 50))) {
                while (usedXPositions.contains(x)) {
            ...
            // Rơi vật phẩm ID 749
            if (Util.isTrue(50 + levelPlayer / 2, 100)
                    || (player.actived() && Util.isTrue(dropRate + levelPlayer / 2, 50))) {
                while (usedXPositions.contains(x)) {
                    x += 10;
                }
        ...
        if (MapService.gI().isMapUpPorata(mapid)) {
            ...
            } else if (Util.isTrue(dropRate4, 500)) {
        ...
        if (MapService.gI().isMap3Planets(mapid)) {
            if (Util.isTrue(1, 20)) {
                ...
                if (player.nPoint.tlGold > 0) {
                    vang = (int) (vang * (1 + player.nPoint.tlGold / 100.0));
                }
                if (vang < 1000) {
        ...
        if (MapService.gI().isMapRiengTu(mapid)) {
            ...
            if (Util.isTrue((int) tileDrop, 3000)) {
                ...
                // ChatGlobalService.gI().ThongBaoRoiDo(player, player.name + " vừa nhặt được "
                // + it.itemTemplate.name + " sét kích hoạt tại " + this.zone.map.mapName + "
        ...
        if (Util.isTrue(1, 100) || (player.nPoint.isDoSPL && Util.isTrue(5, 100))
                || (player.actived() && Util.isTrue(3, 100))) {
        ...
        // Da nang cap
        if (Util.isTrue(1, 400) || (Util.isTrue(1, 300) && MapService.gI().isMapTuongLai(mapid))
                || (player.actived() && Util.isTrue(1, 200))) {
            int rand = Util.nextInt(0, 4);
            ItemMap it = new ItemMap(zone, 220 + rand, 1, x, yEnd, player.id);
            it.options.add(new Item.ItemOption(71 - rand, 0));
            list.add(it);
        ...
        // Manh thien su
        if ((Util.isTrue(1, 1000) || (player.actived() && Util.isTrue(1, 300)))
