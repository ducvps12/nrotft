...
public class ToSuKaio extends Npc {
    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            String message = String.format(
                    "Tập luyện với Tổ sư Kaio sẽ tăng %s sức mạnh mỗi phút, có thể tăng giảm tùy vào khả năng đánh quái của con",
            ...
            case 1 -> {
                if (player.inventory.gold >= OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER) {
                    ...
                    Service.gI().sendThongBao(player, "Bạn không đủ vàng để mở, còn thiếu "
                            + Util.numberToMoney(OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER - player.inventory.gold)
                            + " vàng");
                }
            }
        }
    ...
    private void showAutoTrainingRegistrationMenu(Player player) {
        String message = String.format(
                "Đăng ký để mỗi khi Offline quá 30 phút, con sẽ được tự động luyện tập với tốc độ %s sức mạnh mỗi phút",
