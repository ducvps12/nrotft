                        ...
                        case ConstNpc.BUA_HO_TRO -> {
                            if (select == 0) {
                                if (player.inventory.getGem() >= 5) {
                                    ...
                                    if (!player.itemTime.isUseKilis || currentTime > (player.itemTime.lastTimeUseKilis
                                            + player.itemTime.timeLengthKilis)) {
                                        player.itemTime.lastTimeUseKilis = currentTime;
                                        player.itemTime.timeLengthKilis = addedTime;
                                        player.itemTime.isUseKilis = true;
                                    } else {
                                        long remainingTime = (player.itemTime.lastTimeUseKilis
                                                + player.itemTime.timeLengthKilis) - currentTime;
