...
public class InventoryService {
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        if (player.isPet && (item.template.type == 21 || item.template.type == 27)) {
            Service.gI().sendThongBaoOK(((Pet) player).master, "cc");
            return sItem;
        }
        ...
        checkOption231(item);
        int index = -1;
        // Kiểm tra nếu item là sách tuyệt kỹ thì luôn vào slot 9 (ô sách tuyệt kỹ)
            ...
            case Inventory.TYPE_NORMAL_BOX -> {
                ...
                if (item.isNotNullItem()) {
                    ...
                    if (item.template.type >= 0 && item.template.type <= 5 || item.template.type == 32) {
                        Item itemBody = player.inventory.itemsBody
                                .get(item.template.type == 32 ? 6 : item.template.type);
                        if (!itemBody.isNotNullItem()) {
                            if (item.template.gender == player.gender || item.template.gender == 3) {
                                ...
                                if (powerRequire <= player.nPoint.power) {
                                    player.inventory.itemsBody.set(item.template.type == 32 ? 6 : item.template.type,
                                            item);
    ...
    public void sendItemBody(Player player) {
        ...
        try {
            msg = new Message(-37);
            msg.writer().writeByte(0);
            // Khi fusion, gửi head gốc để hiển thị đúng avatar trong UI inventory
    ...
    public Item finditemLyNuocMia(Player player, int soluong) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1614 || item.template.id == 1615)
