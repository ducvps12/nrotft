...
public class ClanService {
    ...
    private void acceptAskJoinClan(Player player, int clanMessageId) {
        ...
        if (existInClan) {
            Service.gI().sendThongBao(player, "Người này đã ở trong bang.");
            return;
        }
        ...
        // nếu player offline thì load từ DB
        if (pl == null) {
            pl = NDVSqlFetcher.loadById(plxinvao);
        }

        if (pl == null) {
            Service.gI().sendThongBao(player, "Không tìm thấy người chơi");
            return;
        }

        if (pl.iDMark != null && pl.iDMark.isHoldBlackBall()) {
        ...
        if (pl.clan != null) {
            cmg.text = pl.name + " đã vào bang khác.";
            clan.sendMyClanForAllMember();
            return;
        }
        ...
        // nếu player online thì gửi packet
