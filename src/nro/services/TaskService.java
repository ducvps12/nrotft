...
public class TaskService {
    ...
    private String getKolTask(Player player) {
        if (player.playerTask.kolTask != null && player.playerTask.kolTask.template != null) {
            return "\n\n                  [Nhiệm Vụ Kol]"
                    ...
                    + "\n-Hoàn thành: " + player.playerTask.kolTask.count + "/"
                    + player.playerTask.kolTask.template.max_count + " ("
                    + player.playerTask.kolTask.getPercentProcess() + "%)";
        }
        return "";
    }
    ...
    public int getIdTask(Player player) {
        if (player.isPet && !player.isBot && !player.isClone || player.isBoss || player.playerTask == null
                ...
                switch (player.playerTask.sideTask.level) {
                    case ConstTask.EASY:
                        ...
                        if (EventManager.TEACHERS_DAY && quetre != null) {
                            ...
                            boolean hasExpiry = quetre.itemOptions.stream()
                                    .anyMatch(opt -> opt.optionTemplate.id == 73);
                        }
                        ngocBi = 708;
