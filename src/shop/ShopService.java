...
public class ShopService {
    ...
    public void takeItem(Player player, byte type, int tempId) {
        ...
        } else if (tagName.equals("SANTA_GUILD"))

        {
            buyItemGuild(player, tempId);
        }
  ...
  public void showConfirmSellItem(Player pl, int where, int index) {
    ...
    if (item != null && item.isNotNullItem()) {
        ...
        if (item.template.id == 555
                || item.template.id == 557
                || item.template.id == 559) {

            sendConfirmMessage(pl, where, index, item.template.name, 20);
            return;
        }

        // ======= 25 THỎI =======
        if (item.template.id == 556
                || item.template.id == 568
                || item.template.id == 560) {

            sendConfirmMessage(pl, where, index, item.template.name, 25);
            return;
        }
        ...
        if (item.template.id == 561) {
            ...
            sendConfirmMessage(pl, where, index, item.template.name, 30);
            return;
        }

        // ======= 50 THỎI =======
        if (item.template.id == 562
                || item.template.id == 564
                || item.template.id == 566) {

            sendConfirmMessage(pl, where, index, item.template.name, 50);
            return;
        }

        // ======= 40 THỎI =======
        if (item.template.id == 563
                || item.template.id == 565
                || item.template.id == 567) {

            sendConfirmMessage(pl, where, index, item.template.name, 40);
            return;
        }
...
// ================= HÀM PHỤ =================
private void sendConfirmMessage(Player pl, int where, int index, String itemName, int goldAmount) {

    String text = "Bạn có muốn bán\n"
            + itemName
            + "\nvới giá " + goldAmount + " thỏi vàng?";
   ...
   public void sellItem(Player pl, int where, int index) {
    ...
    Item item = (where == 0)
            ? pl.inventory.itemsBody.get(index)
            : pl.inventory.itemsBag.get(index);
    ...
    if (item.template.id == 555
            || item.template.id == 557
            || item.template.id == 559) goldBarAmount = 20;

    // 25
    if (item.template.id == 556
            || item.template.id == 568
            || item.template.id == 560) goldBarAmount = 25;

    // 30
    if (item.template.id == 561) goldBarAmount = 30;

    // 50
    if (item.template.id == 562
            || item.template.id == 564
            || item.template.id == 566) goldBarAmount = 50;

    // ======= 40 THỎI (563, 565, 567) =======
    if (item.template.id == 563
            || item.template.id == 565
            || item.template.id == 567) goldBarAmount = 40;
    ...
    if (goldBarAmount > 0) {
        ...
        goldBar.quantity = goldBarAmount;
        ...
        Service.gI().sendThongBao(pl,
                ...
                + " nhận được " + goldBarAmount + " thỏi vàng");
    ...
    private void getItemSideBoxLuckyRound(Player player, List<Item> items, byte type, int index) {  // dung lai
        if (items == null) {
            return;
        }
    ...
    private void buyItemHD(Player player, int itemTempId) {
        ...
        // 2. Xử lý đổi Thức Ăn theo loại món đồ (Áo, Quần, Găng, Giày, Nhẫn)
        if (item.template.level == 14) {
            int foodIdRequired = -1;

            // Ánh xạ Type món đồ sang ID thức ăn
            // type 0: Áo, 1: Quần, 2: Găng, 3: Giày, 4: Nhẫn/Phụ kiện
            switch (item.template.type) {
                case 0:
                    foodIdRequired = 663;
                    break; // Áo đổi bằng thức ăn 663
                case 1:
                    foodIdRequired = 664;
                    break; // Quần đổi bằng thức ăn 664
                case 2:
                    foodIdRequired = 665;
                    break; // Găng đổi bằng thức ăn 665
                case 3:
                    foodIdRequired = 666;
                    break; // Giày đổi bằng thức ăn 666
                case 4:
                    foodIdRequired = 667;
                    break; // Nhẫn đổi bằng thức ăn 667
                default:
                    Service.gI().sendThongBao(player, "Vật phẩm không hợp lệ");
                    return;
            }

            final int requiredId = foodIdRequired;
            Item doAn = player.inventory.itemsBag.stream()
                    .filter(it -> it != null && it.template != null
                            && it.template.id == requiredId
                            && it.quantity >= 99)
            ...
            if (doAn != null) {
                ...
                Service.gI().sendThongBao(player, "Bạn cần có 99 thức ăn ID " + requiredId + " để đổi món này!");
                return;
            }
        }

        // 3. Khấu trừ tiền (Vàng/Ngọc/Ruby)
        if (!subMoneyByItemShopV2(player, is)) {
            return;
        }

        // 4. Kiểm tra điều kiện mặc đồ Thần (Level 13)
        Item dothan = player.inventory.itemsBody.stream()
                .filter(it -> it != null && it.template != null && it.template.level == 13)
                .findFirst().orElse(null);

        if (dothan == null) {
            Service.gI().sendThongBao(player, "Bạn phải mặc ít nhất 1 món đồ Thần trên người.");
            return;
        }

        // 5. Tính toán chỉ số ngẫu nhiên (Param)
        int param = 0;
        if (item.template.level == 14) {
        ...
        if (!item.itemOptions.isEmpty()) {
            for (ItemOption ios : item.itemOptions) {
                if (item.template.level == 14 && InventoryService.gI().optionCanUpgrade(ios.optionTemplate.id)
                        && param > 0) {
                    int paramUpdate = ios.param + (ios.param * param) / 100;
                    itemoptions.add(new ItemOption(ios.optionTemplate.id, paramUpdate));
                } else if (ios.optionTemplate.id != 164) {
                    itemoptions.add(new ItemOption(ios.optionTemplate.id, ios.param));
                }
            }
        } else {
            itemoptions.add(new ItemOption(73, 0));
        }
    ...
    private void buyItemGuild(Player player, int itemTempId) {
        ...
        if (pointExchange <= 0) {
            Service.gI().sendThongBao(player, "Item này chưa cấu hình điểm bang");
            return;
        }

        // 🔥 DÙNG ĐIỂM CÁ NHÂN
        if (player.clan_point < pointExchange) {
            Service.gI().sendThongBao(player,
                    "Bạn còn thiếu " + (pointExchange - player.clan_point) + " điểm bang cá nhân");
            return;
        }

        // Trừ điểm cá nhân
        player.clan_point -= pointExchange;
        ...
        Service.gI().sendThongBao(player,
                "Đổi thành công " + item.template.name);

        // gửi lại tiền/điểm nếu client có hiển thị
        Service.gI().sendMoney(player);
